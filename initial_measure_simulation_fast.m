fprintf('Init Measure Sim NEW (FAST)\n');
% You don't need to choose model for this experiment, ONLY ONE MODEL
...
for i_stage = 1:n_bins
    ...
    fprintf('%i-th bin: VDD: %4.4f - VTN: %4.4f - VTP: %4.4f\n', i_stage, Vd, Vtn, Vtp);
    fprintf(fid,'.param sigma=0 l=%d clock_b%i_ini=%4.4fn cl=%d vd_b%i_ini=%d Vtn_b%i_ini=%d Vtp_b%i_ini=%d rl=%d fanout=%3.2f Cout=%d ', ...
                                    L, i_stage, 50000, C0, i_stage, Vd, i_stage, Vtn, i_stage, Vtp, R, fanout-1, Cout);
...
for i=1:1:n_bins
    ...
    [Delay_i(i), status] = str2num(avgdly);
    if( (status==0) || (Delay_i(i)>1e-4) || (Delay_i(i)<0) )
        disp('initial simulation failed');
        f_i=1;
        %break;
    end
    tline=fgetl(fid);
...
Delay_i = Delay_i./calib_delay;

fprintf('Initial Simulation Done\n');
